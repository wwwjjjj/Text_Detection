...
def get_ce_loss(gt_score, pred_score):
	#return F.binary_cross_entropy(pred_score,gt_score)
	return bce(pred_score,gt_score)/(torch.sum(pred_score)+1e-7)
...
def get_smoth_loss(gt_geo, pred_geo):
	d1_gt, d2_gt, d3_gt, d4_gt, angle_gt = torch.split(gt_geo, 1, 1)
	d1_pred, d2_pred, d3_pred, d4_pred, angle_pred = torch.split(pred_geo, 1, 1)
	iou_loss_map=F.smooth_l1_loss(d1_pred,d1_gt)+F.smooth_l1_loss(d2_pred,d2_gt)+F.smooth_l1_loss(d3_pred,d3_gt)+F.smooth_l1_loss(d4_pred,d4_gt)
	iou_loss_map=iou_loss_map/4
	angle_loss_map = 1 - torch.cos(angle_pred - angle_gt)
	return iou_loss_map, angle_loss_map
...
class EAST_Loss(nn.Module):
	...
	def forward(self, gt_score, pred_score, gt_geo, pred_geo, ignored_map):
		...
		classify_loss = get_ce_loss(gt_score, pred_score * (1 - ignored_map))
		iou_loss_map, angle_loss_map = get_geo_loss(gt_geo, pred_geo)
		#iou_loss_map, angle_loss_map = get_smoth_loss(gt_geo, pred_geo)
		...
		loss_stats = {'loss': geo_loss + classify_loss,
					  'geo_loss':geo_loss,
					  'classify_loss':classify_loss}  # 'wh_loss':wh_loss,

		return geo_loss + classify_loss,loss_stats
